...
def test_revit_cli():
    ...
    # Verify result.json and Excel export content
    if not os.path.exists("result.json"):
        print("❌ Missing result.json output!")
        return False

    if not os.path.exists("空間裝修與門窗匯出表.csv"):
        print("❌ Missing 空間裝修與門窗匯出表.csv Excel export output!")
        return False

    print("   ✅ [PASS] 成功即時推送圖塊資訊至 Excel 檔案 (空間裝修與門窗匯出表.csv)！")
