...
def main():
    ...
    parser.add_argument("--out-excel", help="即時推送至 Excel 檔案路徑 (.csv / .xlsx)")
    ...
    if args.out_excel:
        export_to_excel(results, args.out_excel, db)
...
def export_to_excel(results: List[Dict[str, Any]], excel_filepath: str, rule_db: SpatialRuleDB):
    file_exists = os.path.exists(excel_filepath)
    with open(excel_filepath, 'a', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["圖塊/空間名稱", "粉刷編號", "門寬", "門窗編號", ".md檔備註"])
        for r in results:
            rule = rule_db.get_rule_for_space(r.get("room_name", ""))
            notes = rule.get("notes", "") if rule else "無對應規範備註"
            finish_code = rule.get("wall_finish", "").split()[0] if (rule and rule.get("wall_finish")) else ""
            door_code = r.get("door", "")
            w_val = int(r.get('width', 90)) if r.get('width') else 90
            door_width_str = f"{w_val} cm"
            writer.writerow([r.get("room_name", ""), finish_code, door_width_str, door_code, notes])
            print(f"   [EXCEL PUSH OK] 房間/圖塊 '{r.get('room_name')}' -> 粉刷: {finish_code} | 門寬: {door_width_str} | 門窗: {door_code} | 備註: {notes} -> 推送至 Excel: {excel_filepath}")
